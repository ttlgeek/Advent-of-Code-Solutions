...
def part1(inputs):
  ...
  while phaseSettings != initialPhaseSettings:
    ...
    for i in range(1, 5):
      lastIndex, output, state = intCodeProgram(values, [phaseSettings[i],  thrusterOutputs[i - 1]])
...
def part2(inputs):
  ...
  while phaseSettings != initialPhaseSettings:
    ...
    lastIndex, output, state = intCodeProgram(inputs, [phaseSettings[ampIndex], 0], 0, True)
    states[0] = state, lastIndex, output
    ampIndex = 1
    while True:
      lastState, lastIndex, _ = states[ampIndex]
      args = [phaseSettings[ampIndex], states[ampIndex - 1 if ampIndex != 0 else 4][2]] if not passedFirstLoop else [states[ampIndex - 1 if ampIndex != 0 else 4][2]]
